...
class BookRecommendation(BaseModel):
  ...
  rating: str = Field(description="평점 (1-5)")
...
def get_book_search(search_keyword: Optional[str] = None) -> Dict[str, Any]:
  """최신 IT 도서 정보를 가져온다. 키워드가 없으면 신간 도서 목록을 가져온다."""
  if search_keyword:
...
book_data_agent = Agent(
  name="book_data_agent",
  model="gemini-2.5-flash",
  description="최신 도서 정보를 조회하는 에이전트",
  instruction="""
  사용자의 관심사에 맞는 최신 도서 정보를 조회하세요.
  get_book_search 도구를 사용하여 현재 인기 있는 도서들을 조회하고, 사용자의 선호도에 맞는 도서들을 선별하세요.
  """,
...
structured_output_agent = Agent(
  ...
  각 도서에 대해 추천 이유를 작성하고, 전체 추천 도서 수를 'total_count'에 포함하세요.
  """,
...
root_agent = Agent(
  ...
  처리 순서:
  1. 먼저 book_data_agent 에이전트를 사용하여 사용자의 관심사에 맞는 최신 도서 정보를 조회합니다.
  2. 수집된 정보를 structured_output_agent 에이전트에게 전달하여 BookList 형식으로 구조화된 추천 목록을 생성합니다.
  3. 최종 결과를 사용자에게 전달합니다.
  
  각 에이전트의 역할:
  - book_data_agent: IT 도서 베스트셀러 정보 조회
  - structured_output_agent: 구조화된 BookList 형식으로 출력 생성
  """,
